...
strip connectors, four power-only JST connectors and the 5V screw terminal.
No fuses, no 74AHCT125 level shifter, no series resistors, no capacitors --
GPIO data goes straight to the JST data pins at 3.3 V. Board is 85 x 70 mm.
Top view:
  ...
  | [J7]    data traces           [J2]    |
  | [J8]  . . . Pi header . . .   [J3]    |
  | [J9]  [ Pi Zero 2 W below ]   [J4]    |
  | [J10]                         [J5]    |
  | ========== +5V ================       |
  +---------------------------------------+

J1 (5V in) is horizontally centered on the top edge. J2-J5 on the right
edge carry +5V / DATA / GND; J7-J10 on the left edge are the SAME socket
but power-only: pin 1 = +5V, pin 3 = GND, pin 2 unconnected.
...
Layer scheme (kept deliberately readable):
  ...
  - F.Cu: a +5V ring around all four board edges (plus an arm to the Pi
    5 V pins) and the four data traces, all visible from the top.
...
PJX = 5.5                           # power-only JST column (courtyard clears H1/H3)
JY = [11.5 + 14 * i for i in range(4)]   # JST pin-1 rows
J1X = 42.5 - 2.54                   # J1 pad-1 x so the two pads straddle centerline

# ---------------- components ----------------
j1 = place("TerminalBlock_Phoenix",
           "TerminalBlock_Phoenix_MKDS-3-2-5.08_1x02_P5.08mm_Horizontal",
           "J1", J1X, 8.0, rot=0, value="5V IN")
...
# power-only JSTs on the left edge, mirroring J2-J5: same socket, same pin
# order (+ top, G bottom), but pin 2 (data) is left unconnected.
for i in range(4):
    j = place("Connector_JST", "JST_XH_B3B-XH-A_1x03_P2.50mm_Vertical",
              f"J{i+7}", PJX, JY[i], rot=270, value="PWR")
    setnets(j, {"1": N_5V, "3": N_GND})
...
# Front: ONLY +5V -- a ring around all four edges, everything you see is +5V.
zone(rect(0, 0, W, 12.0), N_5V, F_Cu, priority=4)      # top edge, fed by J1
zone(rect(73.0, 0, W, H), N_5V, F_Cu, priority=2)      # right edge -> JST pin 1
zone(rect(0, 63.0, W, H), N_5V, F_Cu, priority=1)      # bottom edge
# left edge (power-only JSTs) widened above y=44 into an arm to the Pi 5V pins
zone([(0, 0), (20.2, 0), (20.2, 44.0), (12.0, 44.0), (12.0, H), (0, H)],
     N_5V, F_Cu, priority=3)
...
# ---------------- silkscreen ----------------
silk("gLED Controller simple", 40, 30.3, 1.6)
silk("+", J1X, 14.8, 1.5); silk("-", J1X + 5.08, 14.8, 1.5)
silk("5V IN", 42.5, 17.3, 1.0)
silk("NO FUSES - FUSE SUPPLY EXTERNALLY", 42.5, 20.5, 1.0)
silk("3V3 DATA - KEEP STRIP LEADS SHORT", 42.5, 23.0, 1.0)
silk("GND = SOLID PLANE ON BOTTOM", 42.5, 25.5, 1.0)
silk("LEFT JST = POWER ONLY (NO DATA)", 42.5, 28.0, 1.0)
silk("+5V", 58.0, 6.0, 1.5)
silk("+5V", 10.5, 21.0, 1.5, angle=90)
silk("+5V", 80.8, 33.0, 1.5, angle=90)
silk("+5V", 42.5, 66.5, 1.5)
for i, (g, _, s) in enumerate(ROWS):
    silk(f"{s} GPIO{g}", 62.5, JY[i] + 7.3, 1.0)
    silk("+", JX - 4.5, JY[i], 1.0)
    silk("D", JX - 4.5, JY[i] + 2.5, 1.0)
    silk("G", JX - 4.5, JY[i] + 5.0, 1.0)
for i in range(4):  # power-only column: x dodges the H1 mounting hole at row 3
    lx = 9.2 if i == 2 else 10.0
    silk("+", lx, JY[i], 1.0)
    silk("G", lx, JY[i] + 5.0, 1.0)

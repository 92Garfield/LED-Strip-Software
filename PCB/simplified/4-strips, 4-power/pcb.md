...
the 5 V screw terminal (J1, horizontally centered on the top edge), four JST
strip connectors (J2–J5, right edge), four **power-only** JST connectors
(J7–J10, left edge) and the 2×20 socket for the Pi Zero 2 WH (J6).
85 × 70 mm, 2 layers, 2 oz copper. Same carrier concept as the full board:
the socket sits on the **back**, the Pi hangs underneath on M2.5 standoffs
with its pins pointing up into it.

J7–J10 use the **same JST XH socket** as J2–J5 (one part type to buy), but
only +5 V (pin 1, top) and GND (pin 3, bottom) are wired — the middle data
pin is unconnected. Use them to inject power mid-strip or at the far end
of long strips.

Only 10 solder parts, all through-hole — a 20-minute soldering job.
...
- **Front layer = only +5V areas + the four data traces.** Any copper you
  see on top is +5V: a **ring around all four board edges** fed by J1 at
  the top center, plus an arm from the left edge to the Pi's 5 V pins
  (header pins 2/4). The ring feeds every JST `+` pin on both sides. Each
  edge carries a `+5V` silkscreen label.
...
The left-edge column J7 (top) → J10 (bottom) mirrors the right column but
carries **no data**: `+` on the top pin, `G` on the bottom pin, middle pin
open. The silkscreen marks the polarity at each connector.
...
```
cd "PCB/simplified/4-strips, 4-power"
